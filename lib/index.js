...
const concat = async (outfile, options) => {
    ...
    try {
        ...
        if (!options.includeCurrent) {
            const package = JSON.parse(fs.readFileSync(path.join(path.normalize(options.start), 'package.json')));
        ...
        Object.keys(json).forEach((package) => {
            ...
            if (typeof licensePath === 'string') {
                const licenseFileBasename = path.basename(licensePath, path.extname(licensePath)).toUpperCase();
